...
local FileIcon = {
  init = function(self)
    self.icon, self.icon_color = require('mini.icons').get('filetype', vim.bo.filetype)
    self.icon_color = vim.api.nvim_get_hl(0, { name = self.icon_color, link = false }).fg
  end,
  provider = function(self)
...
local LSPActive = {
  ...
  provider = function()
    ...
    local total = #vim.lsp.get_clients()
    return icons.lsp .. '[' .. unpack(names) .. ':' .. total .. ']'
  end,
...
local DefaultStatusline = {
  ...
  LSPActive,
  Space,
  -- disable it when enabling noice
  -- LspProgress,
  Align,
